...
calc_wad <- function(data, tax_id = c(), sample_id = c(), frac_id = c(),
  ...
  data[, fAt := rA * fA, by = c(tax_id, sample_id, frac_id)
       ][, tA := sum(fAt), by = c(tax_id, sample_id)
         ][, weight := fAt / tA, by = c(tax_id, sample_id, frac_id)]
  # calculate WADs
